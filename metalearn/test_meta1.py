...
meta_input_state =  tf.placeholder(tf.float32, [None, 1])
meta_groundtruth =  tf.placeholder(tf.float32, [None, 1344])

meta_w1 = tf.Variable(tf.truncated_normal([1, 64], stddev=0.1), trainable=True)
meta_b1 = tf.Variable(tf.truncated_normal([64], stddev=0.01), trainable=True)
meta_fc1 = tf.matmul(meta_input_state, meta_w1) + meta_b1

meta_w2 = tf.Variable(tf.truncated_normal([64, 1344], stddev=0.1), trainable=True)
meta_b2 = tf.Variable(tf.truncated_normal([1344], stddev=0.01), trainable=True)
...
log_file = open("data/test_meta1_1/log1.txt", "wt")
...
for epoch in range(2001):
    ...
    for envn in range(16):
        agent_w1.assign(np.reshape(o_param[envn][:128], (4, 32)))
        agent_b1.assign(o_param[envn][128:160])
        agent_w2.assign(np.reshape(o_param[envn][160:1184], (32, 32)))
        agent_b2.assign(o_param[envn][1184:1216])
        agent_w3.assign(np.reshape(o_param[envn][1216:], (32, 4)))
    ...
    if epoch % 50 == 0:
        saver.save(sess, "data/test_meta1_1/log1_" + str(epoch) + ".ckpt")
