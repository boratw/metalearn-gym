...
input_state = tf.placeholder(tf.float32, [None, 6])
input_action = tf.placeholder(tf.float32, [None, 3])
input_qvalue = tf.placeholder(tf.float32, [None, 1])
global_step = tf.placeholder(tf.int64)

w1 = tf.Variable(tf.truncated_normal([6, 32], stddev=0.1), trainable=True, name="w1")
...
w3 = tf.Variable(tf.truncated_normal([32, 3], stddev=0.1), trainable=True, name="w3")
...
log_file = open("data/test5/log1.txt", "wt")
...
for epoch in range(1001):
    ...
    if epoch % 50 == 0:
        saver.save(sess, "data/test5/log1_" + str(epoch) + ".ckpt")
