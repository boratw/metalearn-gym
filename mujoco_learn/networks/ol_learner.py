...
class TRPOLearner:
    def __init__(self, state_len, action_len, name="", policy_lr=0.01, policy_hidden_len=64, value_lr=0.01, value_hidden_len=128, value_gamma=0.96, nextstate_hidden_len=64, nextstate_lr=0.01,kl_delta=0.01):
        ...
        with tf.variable_scope("TRPOLearner" + name): 
            ...
            self.value_train = tf.train.AdamOptimizer(input_learning_rate).minimize(loss = self.value_loss, var_list = self.value_network.trainable_params)
            self.nextstate_loss = tf.reduce_mean((self.nextstate_network.layer_output - self.input_next_state) ** 2)
            self.nextstate_train = tf.train.AdamOptimizer(input_learning_rate).minimize(loss = self.nextstate_loss, var_list = self.nextstate_network.trainable_params)
    ...
    def get_nextstate_diff(self, input_state, input_action, input_next_state)
        sess = tf.get_default_session()
        output = sess.run(self.nextstate_loss,  {self.input_state : np.array([input_state]), self.input_action : np.array([input_action]), self.input_next_state : np.array([input_next_state])} )
        return output

    def optimize_value_batch(self, input_learning_rate, input_state, input_next_state, input_value):
        sess = tf.get_default_session()
        _, loss = sess.run([self.value_train, self.value_loss], {self.input_learning_rate:input_learning_rate, self.input_state : np.array(input_state), self.input_next_state : np.array(input_next_state),  self.input_value : np.array(input_value)} )
        return loss

    def optimize_nextstate_batch(self, input_learning_rate, input_state, input_action, input_next_state):
        sess = tf.get_default_session()
        _, loss = sess.run([self.nextstate_train, self.nextstate_loss], {self.input_learning_rate:input_learning_rate, self.input_state : np.array(input_state), self.input_action : np.array(input_action), self.input_next_state : np.array(input_next_state)} )
        return loss
