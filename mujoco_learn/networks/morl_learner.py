...
class MORLLearner:
    def __init__(self, state_len, action_len, name="", policy_lr=0.001, policy_hidden_len=64, state_lr=0.01, state_hidden_len=128, gamma=0.95, action_maximum=1.0):
        ...
        with tf.variable_scope("MORLLearner" + name): 
            ...
            self.policy_loss = tf.reduce_mean( -self.new_policy_network.log_prob(self.input_action) * self.input_advantage)
            ...
            self.kl_div = tf.reduce_sum(tf.square(self.policy_network.layer_output_action - self.new_policy_network.layer_output_action))
    ...
    def optimize_policy_batch(self, input_learning_rate, input_state, input_action, input_advantage):
        sess = tf.get_default_session()
        ...
        _, loss = sess.run([self.policy_train, self.policy_loss], {self.input_state : np.array(input_state), self.input_action : np.array(input_action),
                self.input_advantage : np.array(input_advantage), self.input_learning_rate : input_learning_rate} )
    ...
    def get_next_with_policy(self, input_state):
        sess = tf.get_default_session()
        state, reward = sess.run([self.nextstate_network.layer_foggy_state, self.nextstate_network.layer_output_reward],
    ...
    def get_action_collecting(self, input_state):
        ...
        action = [ a * 0.5 + random.gauss(0.0, self.action_maximum * 0.25) for a in action[0]]
        return action
