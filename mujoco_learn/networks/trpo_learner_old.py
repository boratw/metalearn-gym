...
class TRPOLearner:
    def __init__(self, state_len, action_len, name="", policy_lr=0.01, policy_batch = 16, policy_hidden_len=64, next_state_hidden_len=64, next_state_lr = 0.01):
        ...
        with tf.variable_scope("TRPOLearner" + name): 
            ...
            #input_advantage_avg = tf.reduce_mean(self.input_advantage)
            ...
            self.new_policy_loss = tf.reduce_sum(((self.new_policy_network.mu - self.input_action) ** 2) * input_advantage_clipped * self.input_learning_rate)
            ...
            self.next_state_loss = tf.reduce_mean((self.next_state_network.layer_output - self.input_next_state) ** 2 * self.input_learning_rate)
    ...
    def get_action_stochastic(self, input_state, input_sigma):
        sess = tf.get_default_session()
        output = sess.run(self.policy_network.walk, {self.input_state : np.array([input_state]), self.input_sigma : input_sigma})
        return output
    ...
    def update_next_state(self, input_state, input_action, input_next_state, learning_rate):
        sess = tf.get_default_session()
        loss = sess.run(self.next_state_train, {self.input_state : np.array(input_state),  self.input_action : np.array(input_action),
            self.input_next_state : np.array(input_next_state), self.input_learning_rate : learning_rate} )
        return loss

    def init_from_maml(self, maml):
        sess = tf.get_default_session()
        update = [ [ tf.assign(target, source) for target, source in zip(self.policy_network.trainable_params, maml.new_policy_network.trainable_params)  ], 
            [ tf.assign(target, source) for target, source in zip(self.next_state_network.trainable_params, maml.next_state_network.trainable_params)  ]]
