...
class MORLLearner:
    def __init__(self, state_len, action_len, name="", 
        ...
        qvalue_lr = 0.01, qvalue_hidden_len = 64, 
        ...
        with tf.variable_scope("MORLLearner" + name): 
            ...
            self.nextstate_sto2_network = StateEstimator("state", state_len, action_len, state_hidden_len, hidden_nonlinearity=tf.nn.leaky_relu, 
                input_state_tensor=self.input_state, input_action_tenser=self.policy_network.layer_foggy2_action, reuse=True)
            ...
            self.qvalue_policy_std_network = MLP("qvalue", state_len, 1, qvalue_hidden_len, hidden_nonlinearity=tf.nn.leaky_relu, reuse=True,
                input_tensor=self.input_state, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.new_policy_network.get_dist(self.input_action_std) )
            ...
            self.qvalue_loss = tf.reduce_mean(tf.nn.leaky_relu((self.input_qvalue - self.qvalue_network.layer_output)) ** 2)
            self.qvalue_train = tf.train.AdamOptimizer(qvalue_lr).minimize(loss = self.qvalue_loss, var_list = self.qvalue_network.trainable_params)

            self.policy_loss = tf.reduce_mean( -self.qvalue_policy_std_network.layer_output + self.new_policy_network.regularization_loss)
    ...
    def optimize_qvalue_batch(self, input_state, input_action, input_qvalue):
        sess = tf.get_default_session()
        _, loss = sess.run([self.qvalue_train, self.qvalue_loss], {self.input_state : np.array(input_state), self.input_action : np.array(input_action),
                self.input_qvalue : input_qvalue} )
        return loss

    def optimize_policy_batch(self, input_state, input_action_std):
        sess = tf.get_default_session()
        overfitted = False
        while True:
            ...
            if div > self.update_kl_div or np.isnan(div):
                overfitted = True
    ...
    def get_nexts_sto2_with_policy(self, input_state):
        sess = tf.get_default_session()
        state, reward = sess.run([self.nextstate_sto2_network.layer_output_state, self.nextstate_sto2_network.layer_output_reward],
                {self.input_state : np.array(input_state)} )
        return state, np.reshape(reward, -1)
    ...
    def get_action_stochastic2(self, input_state):
        sess = tf.get_default_session()
        action = sess.run(self.policy_network.layer_foggy2_action,
                {self.input_state : np.array([input_state])} )
        return action[0]
