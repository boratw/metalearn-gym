...
class MAMLLearner:
    def __init__(self, state_len, action_len, name = "", policy_lr=0.01, policy_batch = 16, meta_policy_lr = 0.01, policy_hidden_len=64, next_state_hidden_len=64, next_state_lr = 0.01, meta_next_state_lr = 0.1):
        with tf.variable_scope("MAMLLearner" + name): 
            ...
            self.next_state_meta_network = MLP("next_state_meta", state_len, state_len, next_state_hidden_len, input_tensor=self.input_state,
                hidden_nonlinearity=tf.nn.leaky_relu, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.input_action)
            self.value_network = MLP("value", state_len, 1, next_state_hidden_len, input_tensor=self.input_next_state,
                hidden_nonlinearity=tf.nn.leaky_relu)
            self.value_meta_network = MLP("value_meta", state_len, 1, next_state_hidden_len, input_tensor=self.input_next_state,
                hidden_nonlinearity=tf.nn.leaky_relu)
            ...
            self.value_initialize = [ tf.assign(target, source) 
                for target, source in zip(self.value_network.trainable_params, self.value_meta_network.trainable_params)  ]

            self.after_policy_network = MLP("nextstate", state_len, state_len, next_state_hidden_len, input_tensor=self.input_state, reuse=True,
                hidden_nonlinearity=tf.nn.leaky_relu, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.new_policy_network.mu)
            self.next_value_network = MLP("value", state_len, 1, next_state_hidden_len, input_tensor=self.after_policy_network.layer_output,
                hidden_nonlinearity=tf.nn.leaky_relu, reuse=True)
            self.cur_value_network = MLP("value", state_len, 1, next_state_hidden_len, input_tensor=self.input_state,
                hidden_nonlinearity=tf.nn.leaky_relu, reuse=True)


            self.new_policy_loss = tf.reduce_mean( -self.next_value_network.layer_output * self.input_learning_rate)
            ...
            self.value_loss = tf.reduce_mean((self.value_network.layer_output - self.input_advantage) ** 2)
            self.value_train = tf.train.AdamOptimizer(next_state_lr).minimize(loss = self.value_loss, var_list = self.value_network.trainable_params)
            self.value_update = [ tf.assign(target, target + (source - target) * meta_next_state_lr) 
                for target, source in zip(self.value_meta_network.trainable_params, self.value_network.trainable_params)  ] 

            self.score = tf.reduce_mean(tf.exp(-tf.abs(self.next_state_network.layer_output - self.input_next_state) / 0.1))
    ...
    def optimize_policy_batch(self, learning_rate, input_state, input_action, input_advantage):

        sess = tf.get_default_session()
        sess.run(self.poilcy_initialize)
        _, loss = sess.run([self.new_policy_update, self.new_policy_loss], {self.input_state : np.array(input_state),  self.input_action : np.array(input_action), self.input_advantage : np.array(input_advantage),
            self.input_learning_rate : learning_rate} )
        div = sess.run( self.kl_div )
        return loss, div

    def update_next_state(self, input_state, input_action, input_advantage, input_next_state):
        sess = tf.get_default_session()
        _, ls, _, lv = sess.run([self.next_state_train, self.next_state_loss, self.value_train, self.value_loss], {self.input_state : np.array(input_state),  self.input_action : np.array(input_action),
            self.input_next_state : np.array(input_next_state), self.input_advantage : np.array(input_advantage)} )
        return ls, lv
    ...
    def init_policy(self):
        sess = tf.get_default_session()
        sess.run([self.poilcy_initialize, self.next_state_initialize, self.value_initialize])

    def update_policy(self):
        sess = tf.get_default_session()
        sess.run([self.policy_update, self.next_state_update, self.value_update])